...
namespace book_a_reading_room_visit.data.Migrations
{
	...
	IF @include_managerial_discretion = 0
		INSERT INTO @seats (seat_id, seat_number) SELECT Id, Number FROM dbo.Seats WHERE  SeatTypeId = 9;
	ELSE
		INSERT INTO @seats (seat_id, seat_number) SELECT Id, Number FROM dbo.Seats WHERE SeatTypeId IN (8, 9);
	...
	SELECT seat_id as Id, seat_number as [Number] FROM @seats WHERE seat_id NOT IN (SELECT seat_id FROM @seats_already_booked);
