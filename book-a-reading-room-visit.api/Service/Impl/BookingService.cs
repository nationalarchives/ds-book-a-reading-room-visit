                    ...
                    if (!string.IsNullOrWhiteSpace(bookingModel.Email))
                    {
                        ...
                        do
                        {
                            ...
                            catch (Exception ex)
                            {
                                ...
                                {
                                    _logger.LogError(ex, $"({Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT")} environment) Customer Confirmation Email send request failed on attempt number {attempts}.  Booking Ref: {bookingModel.BookingReference}, Error : {ex.Message}.  Will retry.");
                                }
